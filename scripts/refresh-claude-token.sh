...
# Claude does not proactively rotate a still-valid token here, so avoid burning
# subscription usage until the current access token has actually expired.
if [ "$BEFORE_EXPIRES_MS" -gt "$NOW_MS" ]; then
  exit 0
fi
