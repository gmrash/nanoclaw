...
Rules:
- For Apple / Find My in `telegram_main`, use the shared persistent browser profile at `/workspace/group/agent-browser-profile`. Do not delete it or create throwaway Apple profiles.
- Before a fresh Apple login, first try the existing browser profile. `agent-browser state load /workspace/group/apple-auth.json` is only a fallback import if the profile is empty or clearly broken.
...
- When starting from a Find My shared link, stay in that Find My browser context and click `Use Apple Account`. Never extract and open a raw `https://idmsa.apple.com/appleauth/auth/authorize/signin?...response_mode=web_message...` URL as a top-level page.
- `account.apple.com` alone is not a successful result. Success means the shared Find My page on `find.apple.com` is authenticated and shows the item/location data.
...
- After login succeeds, treat `/workspace/group/agent-browser-profile` as the primary live Apple session. Also save backup state to `/workspace/group/apple-auth.json` and open the target Find My link once. Do not restart the email/password flow in the same run after a successful login.
...
- For recurring Find My monitoring, do NOT use Claude `CronCreate` / `CronList` / `CronDelete` session-only jobs. They are tied to one Claude session and are unreliable for Apple auth.
- Use the dedicated host service instead:
  - config: `/workspace/group/findmy-monitor-config.json`
  - log: `/workspace/group/scooter_log.txt`
  - browser profile: `/workspace/group/agent-browser-profile`
  - auth state: `/workspace/group/apple-auth.json`
  - control: `systemctl restart nanoclaw-findmy-monitor` and `systemctl status nanoclaw-findmy-monitor --no-pager`
- If the user wants to start or change recurring monitoring, update the config file and restart the service. Do not build an agent-only cron workflow for Apple monitoring.
- If the user manually logs into Apple again, reuse the same shared browser profile so the monitor can continue without rebuilding the workflow.
